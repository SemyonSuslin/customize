Comagic.UI.registerViewController('sitephone', function (settings, tpls) {
    ...
    var TIME_TO_LABEL_ANIMATION = 5000;

    widgets['sitephone_label'].on('render', function () {
        widgets['sitephone_label'].on('show', function () {
            animationTimerId = setInterval(function () {
                widgets['sitephone_label'].getEl().classList.toggle('comagic-js-label--large')
            }, TIME_TO_LABEL_ANIMATION);
        });
        widgets['sitephone_label'].on('hide', function () {
            widgets['sitephone_label'].getEl().classList.remove('comagic-js-label--large');
            clearInterval(animationTimerId);
        });
    });

    widgets['simple_sitephone'].on('render', function () {
        widgets['simple_sitephone'].on('show', function () {
            widgets['sitephone_label'].getEl().classList.remove('comagic-js-label--large');
            clearInterval(animationTimerId);
        });

        widgets['simple_sitephone'].on('hide', function () {
            animationTimerId = setInterval(function () {
                widgets['sitephone_label'].getEl().classList.toggle('comagic-js-label--large')
            }, TIME_TO_LABEL_ANIMATION);
        });
    ...
    //abtesting
    Comagic.trackEvent('Sitephoneh3A', 'Sitephoneh3A');

    widgets['sitephone'].on('show', function () {
        Comagic.trackEvent('Sitephoneh3_A_SHOW', 'Sitephoneh3_A_SHOW');
    });
